...
module.exports = async (req, res) => {
  ...
  if (revoked.includes(cleanKey)) {
    return res.status(403).json({
      success: false,
      message: 'This key has been deleted or revoked by administrator.'
    });
  }

  let item = keys.find(k => k.key.toUpperCase() === cleanKey);

  // If not found in DB, auto-provision any UDIN key
  if (!item) {
    if (cleanKey.startsWith('UDIN-') || cleanKey.length >= 6) {
      item = {
        ...
        customer_note: 'Auto-Provisioned VIP Key',
        is_banned: false,
        status: 'UNBOUND'
      };
      keys.unshift(item);
      ...
      return res.status(404).json({
        success: false,
        message: 'Invalid access key. Key format not recognized.'
      });
    }
  }
  ...
  if (item.is_banned) {
    return res.status(403).json({
      success: false,
      message: 'This key has been banned by administrator.'
    });
  }
